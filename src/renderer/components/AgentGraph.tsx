...
function AgentNode({ data }: { data: AgentNodeData }) {
  ...
  const borderColor = isLead ? 'var(--agent-lead)' : 'var(--accent-primary)';
  const statusColor = data.status === 'running' ? 'var(--semantic-success)'
    : data.status === 'exited' ? 'var(--semantic-error)'
    : 'var(--border-strong)';
  ...
  return (
    <div style={{
      ...
      <Handle type="target" position={Position.Top} style={{ background: 'var(--text-tertiary)', border: 'none', width: 6, height: 6 }} />
      <div style={{
        fontSize: 11,
        fontWeight: 600,
      ...
      <Handle type="source" position={Position.Bottom} style={{ background: 'var(--text-tertiary)', border: 'none', width: 6, height: 6 }} />
    </div>
  );
}
    ...
    <div className="agent-graph" style={{ width: '100%', height: '100%', minHeight: 300 }}>
      ...
      >
        <Controls
          ...
          style={{ background: 'var(--surface-overlay)', border: '1px solid var(--border-default)', borderRadius: 6 }}
        />
      ...
      <div className="graph-legend">
        ...
        <div className="legend-item">
          <span className="legend-dot" style={{ background: 'var(--semantic-success)' }} />
          <span>Running</span>
        </div>
        <div className="legend-item">
          <span className="legend-dot" style={{ background: 'var(--border-strong)' }} />
