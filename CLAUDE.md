...
| History | history-manager, session-digest-service | useHistory, useSessionDigest, HistoryPanel (Cmd+K → "Session history…") | types/history-types.ts | `history:*` |
| Checkpoints | checkpoint-manager, checkpoint-persistence | useCheckpoints, CheckpointsPanel (Cmd+K → "Checkpoints…") | types/checkpoint-types.ts | `checkpoint:*` |
...
| Integrations | integrations/ (integration-manager, connector-registry, connectors/, delivery-queue, attention-policy, message-format, github-service) | useIntegrations, useRemoteDeepLink, IntegrationsPanel, ShipItSheet, IssuePickerSheet | shared/integration-types.ts | `integrations:*`, `session:seedPrompt` |
...
- **History and Checkpoints**: `HistoryManager` and `CheckpointManager` are load-bearing dependencies (`SessionManager` requires `historyManager`; `GitManager` requires `checkpointManager`) with IPC handlers and full persistence. Both now have UI panels opened from the palette: `HistoryPanel` (Cmd+K → "Session history…") browses recorded session transcripts with cross-session content search, export (Markdown/JSON), delete, stats, and retention settings, plus a per-session "while you were away" recap backed by `SessionDigestService` (`session-digest-service.ts`) via `useSessionDigest`; `CheckpointsPanel` (Cmd+K → "Checkpoints…") creates, edits, exports (Markdown/JSON), and deletes checkpoints. Renderer hooks: `useHistory`, `useCheckpoints`.
...
- **Per-account quota mapping**: `resolveClaudeConfigDir` (`quota-service.ts`) picks which Claude config directory (`~/.claude`, or a custom per-account dir) a session's quota is read from, based on the session's working directory. By default (no config) every session reads `~/.claude`. To split quota tracking across multiple Claude accounts on one machine, set `AppSettings.quotaAccountMap` — an ordered list of `{ pathContains, configDir }` rules; the first rule whose `pathContains` is found (case-insensitive, `\`→`/` normalized) as a substring of the working directory wins, and its `configDir` (may start with `~`) is used. Example: `[{ pathContains: '/repositories/work/', configDir: '~/.claude-work' }, { pathContains: '/repositories/personal/', configDir: '~/.claude-personal' }]`. The resolver stays pure/synchronous — `ipc-handlers.ts`'s `getActiveConfigDir()` reads the map once via `settingsManager.getSettings()` and passes it in, rather than doing settings I/O on the hot `getQuota`/`getBurnRate` path.
...
- **Attention cockpit / session-state classifier** (`src/main/session-state/`): the supervisory-cockpit layer classifies each session's live `SessionActivityState` (`initializing` / `working` / `awaiting-approval` / `awaiting-input` / `done` / `errored` / `idle` / `exited`, in `src/shared/ipc-types.ts`) and routes the user's attention. `SessionStateClassifier` (`classifier.ts`) fuses PTY-output patterns (`IProvider.getStateSignals()` + the pure `src/shared/state-detector.ts`), output-idle timing (a quiescence timer), alt-screen state (`alt-screen-tracker.ts`), and the authoritative exit code, with hysteresis. It's wired at `SessionManager`'s single output tap (`wireCliManager`) and broadcasts deltas via the `onSessionStateChanged` / `session:stateChanged` IPC event → `useSessionManager` folds it onto `TabData.activityState`. Renderer surfacing: `SessionRail.mapTabStatus`/`attentionRank` (rich rail chips incl. `needs-approval`), the `useAttentionQueue` hook + `CockpitPanel` overlay (⌘J / Ctrl+J — cross-repo "who needs you" list with Jump/Dismiss + backgrounded toasts), and the StatusBar "N need you" pill. **Shipped**: `SessionManager.setupClassifier` (`session-manager.ts:429-464`) branches on session kind — shell sessions get a byte-tail classifier fed `EMPTY_STATE_SIGNALS` (`:438-440`); agent sessions (Claude Code, Codex) get a `screenDriven: true` classifier backed by a per-session `ScreenModel` (`session-state/screen-model.ts`, wraps `@xterm/headless`) that takes settled snapshots of the terminal's alt-screen buffer and feeds them to the provider's `getStateSignals()` table (`:461-464`) — this is what makes agent CLIs' full-screen, continuously-repainting TUIs classifiable at all, since the output-tail/quiescence model alone can't see past the alt-screen repaint. **Agent sessions also use the terminal bell** alongside screen-driven classification: `BareBellDetector` (`session-state/bell-attention.ts`, escape-aware — BEL also terminates OSC/DCS strings and those never count) watches the same output tap (`:531`); a bare BEL → `awaiting-input` (reason `'bell'`), cleared back to `working` when the user types into the session (`sendInput`). Verified live against Claude Code v2.1.x (`docs/experiments/2026-07-19-bell-attention-probe.md`): rings on turn completion and question prompts, silent while working — **but only when the CLI's bell channel is on** (Claude: `preferredNotifChannel: "terminal_bell"` in the profile's `settings.json`; zero bells otherwise). Errored/exited still come from `SessionMetadata.status`. The screen-aware classification design and its "deferred" history are in `docs/design/2026-07-19-agentic-cockpit-design.md` (see the "Shipped in #202/#207" note at the top callout) — that deferred framing is now historical, not current state.

- **Outbound integrations platform** (`src/main/integrations/`): an event bus + connector layer pushing session events OUT to Telegram / Slack / Discord / a generic HMAC-signed webhook, plus GitHub actions via the `gh` CLI. `IntegrationManager` subscribes to `SessionManager.addStateListener` (main-process fan-out at the same tap as `session:stateChanged`), applies policy — per-event toggles, per-repo mutes (path-prefix match), and an edge-triggered `AttentionPolicy` (notify on entering awaiting-input/awaiting-approval/errored/done, 15s debounce, re-arm on leaving) — then delivers through a per-connector token-bucket `DeliveryQueue` (20/min, Retry-After-aware backoff ×3, bounded drop-oldest). Connectors are stateless `IConnector`s in a `ConnectorRegistry` (mirrors ProviderRegistry); config lives in `settings.integrations` (plaintext tokens — remote-token precedent; keychain is a fast-follow). Notifications carry a deep link `<tunnel-url>/?token=…&session=<id>` ONLY while the tunnel is running (never a dead link); the remote PWA consumes `?session=` via `useRemoteDeepLink` (one-shot focus after hydration, param stripped; desktop ignores it). **Agent-session copy never says "awaiting approval"** — the bell only yields awaiting-input. `GitHubService` (execFile + per-dir mutex, non-interactive env, copied from GitManager) powers **ship-it** (`ShipItSheet` via cockpit done-items + session context menu; preview → explicit Create PR, one PR per branch, `pr-exists:<url>` guard — never auto-creates) and **work intake** (`IssuePickerSheet` palette command: issue → prefilled NewSessionSheet with branch `feat/<n>-<slug>` and the issue body as `initialPrompt`). `initialPrompt` (on `SessionCreateRequest`/`SessionMetadata`, not persisted) is TYPED into the PTY at CLI readiness via `session:seedPrompt` — the once-only guard lives in `SessionManager.seedInitialPrompt` (clears before write, so desktop+remote can't double-type) and it never auto-submits. A fleet digest (default off, skip-when-idle) and per-connector test pings + delivery-status badges round out the `IntegrationsPanel` (Cmd+K → "Integrations…", activity-bar bolt button).
- **Session auto-rename from terminal title**: agent CLIs write `"<glyph> <task summary>"` to the terminal title (OSC 0/2 — Claude Code: braille spinner while working, `✳` settled; verified live 2026-07-19). `OscTitleParser` + `extractTaskTitle` (`src/main/session-state/title-parser.ts`) parse it at the `wireCliManager` output tap; `SessionManager.applyAutoRename` renames the session **only if** the user never explicitly named it (`SessionMetadata.nameIsCustom` — set by an explicit name at create AND by `session:rename`, persisted via the `session-persistence.ts` whitelist) and only when the extracted text changes (glyph churn = no-op, no per-frame emits/disk writes). Shell sessions and junk titles (`.exe` spawn lines, bare `claude`/`Claude Code`) never rename. `OMNIDESK_DEBUG_TITLE` logs every title update.
